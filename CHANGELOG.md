...
- **Fix: "estude o projeto" parava no limite de exploração sem responder** — quando o loop de agente esgota `orunvs.maxIteracoes` ainda lendo arquivos, o OrunVS agora injeta a `SINTESE_PROMPT` e chama o modelo mais uma vez para entregar a resposta/estudo final com base no que já leu (antes terminava só com o aviso "limite atingido", sem resumo)
- **Chat limpo durante exploração**: as narrações intermediárias ("Continuando o estudo...") ficam dobradas num `<details>` colapsado ("⚙ N passo(s) de exploração") — só a resposta final fica em destaque; o cabeçalho "📋 0 arquivo(s) | 0 comando(s)" deixou de aparecer em iterações de leitura pura (só mostra quando há trabalho real)
- **Remoção de lixo de tags**: `limparSobrasAcoes` limpa sobras de `[FILE_READ]`/`[LIST_FILES]`/etc. (tag aberta sem fechamento, blocos malformados, linhas órfãs `path:`/`tool:`) que o `parseAcoes` não pegava e apareciam escritas no final da resposta
- **Fix: "estudar o projeto" acionava o aviso de "nenhuma ação encontrada"** — a palavra "projeto" (e "arquivo"/"código") fazia o auto-retry forçar blocos de ação num pedido que só pede análise. Novos detectores `ehPedidoEstudo`/`ehPedidoImplementacao` em `core.ts`: pedidos de estudo/análise ("estude o projeto", "o que faz o arquivo X", "explique", "leia e resuma") respondem em TEXTO sem retry nem avisos; o auto-retry só dispara para implementação real (criar/editar/corrigir/refatorar). O aviso de "A IA não usou blocos..." agora só aparece se a resposta veio totalmente vazia
